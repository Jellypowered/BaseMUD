...
void player_advance_level(CHAR_T *ch, bool hide)
{
    ...
    add_mana = number_range(2, (2 * char_get_curr_stat(ch, STAT_INT) + char_get_curr_stat(ch, STAT_WIS)) / 5);
    if (!class_table[ch->class].gains_mana)
        add_mana /= 2;
    add_move = number_range(1, (char_get_curr_stat(ch, STAT_CON) + char_get_curr_stat(ch, STAT_DEX)) / 6);
                        ...
                        ROOM_INDEX_T *to_room)
{
    ...
    {
        for (j = 0; j < class_table[i].guild_count; j++)
        {
            FILTER(i != ch->class &&
    ...
    move = sector_table[UMIN(SECT_MAX - 1, from_room->sector_type)].move_loss + sector_table[UMIN(SECT_MAX - 1, to_room->sector_type)].move_loss;
    move /= 2; /* the average */
