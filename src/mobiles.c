...
CHAR_T *mobile_create(MOB_INDEX_T *mob_index)
{
    ...
    if (mob_index->new_format)
    {
        ...
        mob->max_hit = dice(mob_index->hit.number, mob_index->hit.size) + mob_index->hit.bonus;
        mob->hit = mob->max_hit;
        mob->max_mana = dice(mob_index->mana.number, mob_index->mana.size) + mob_index->mana.bonus;
        ...
        if (mob->attack_type == 0)
        {
            ...
            {
            case 1:
                mob->attack_type = ATTACK_SLASH;
                break;
            case 2:
                mob->attack_type = ATTACK_POUND;
                break;
            case 3:
                mob->attack_type = ATTACK_PIERCE;
                break;
            }
        }
        for (i = 0; i < 4; i++)
    ...
    else
    {
        ...
        mob->hitroll = mob_index->hitroll;
        mob->damroll = 0;
        mob->max_hit = (mob->level * 8 + number_range(mob->level * mob->level / 4, mob->level * mob->level)) * 9 / 10;
        mob->hit = mob->max_hit;
...
{
    return IS_NPC(ch) && (EXT_IS_SET(ch->ext_mob, MOB_TRAIN) ||
...
int mobile_get_obj_cost(const CHAR_T *ch, const OBJ_T *obj, bool buy)
{
    ...
    else
    {
        ...
        cost = 0;
        for (itype = 0; itype < shop->buy_count; itype++)
        {
...
void mobile_update(CHAR_T *ch)
{
    ...
    /* Scavenge */
    if (EXT_IS_SET(ch->ext_mob, MOB_SCAVENGER) && ch->in_room->content_first != NULL && number_bits(6) == 0)
    {
        OBJ_T *obj;
        OBJ_T *obj_best;
        int max;
        ...
        {
            if (obj_can_wear_flag(obj, ITEM_TAKE) && char_can_loot(ch, obj) && obj->cost > max && obj->cost > 0)
            {
                obj_best = obj;
                max = obj->cost;
            }
...
bool mobile_wander(CHAR_T *ch)
{
    ...
    if (door < DIR_MAX && (pexit = ch->in_room->exit[door]) != NULL && pexit->to_room != NULL && !IS_SET(pexit->exit_flags, EX_CLOSED) && !IS_SET(pexit->to_room->room_flags, ROOM_NO_MOB) && (!EXT_IS_SET(ch->ext_mob, MOB_STAY_AREA) || pexit->to_room->area == ch->in_room->area) && (!EXT_IS_SET(ch->ext_mob, MOB_OUTDOORS) || !IS_SET(pexit->to_room->room_flags, ROOM_INDOORS)) && (!EXT_IS_SET(ch->ext_mob, MOB_INDOORS) || IS_SET(pexit->to_room->room_flags, ROOM_INDOORS)))
    {
