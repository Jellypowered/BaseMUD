...
DEFINE_SPELL_FUN(spell_acid_rain)
{
    CHAR_T *victim = (CHAR_T *)vo;
    int dam, i;
    ...
    for (i = 8; i > 0; i--)
    {
        ...
        if (saves_spell(level, victim, DAM_ACID))
            dam /= 2;
        act3("{RWaves of {GA{gc{Gi{gd{Gi{gc {BRain {gshower down upon $N!{x",
             "{RWaves of {GA{gc{Gi{gd{Gi{gc {BRain {gcalled by $n shower down upon you!{x",
             "{RWaves of {GA{gc{Gi{gd{Gi{gc {BRain {gcalled by $n shower down upon $N!{x",
...
DEFINE_SPELL_FUN(spell_heat_metal)
{
    ...
    if (saves_spell(level + 2, victim, DAM_FIRE) || IS_SET(victim->imm_flags, RES_FIRE))
    {
