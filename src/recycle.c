...
DEFINE_INIT_FUN(gen_data_init)
{
    GEN_T *gen = obj;
    gen->skill_chosen = calloc(skill_count, sizeof(bool));
    gen->group_chosen = calloc(skill_group_count, sizeof(bool));
}

DEFINE_DISPOSE_FUN(gen_data_dispose)
{
    GEN_T *gen = obj;
    free(gen->skill_chosen);
    free(gen->group_chosen);
}
...
{
    PC_T *pcdata = obj;
    pcdata->learned = calloc(skill_count, sizeof(sh_int));
    pcdata->skill_known = calloc(skill_count, sizeof(sh_int));
    pcdata->group_known = calloc(skill_group_count, sizeof(sh_int));
    pcdata->cond_hours = calloc(COND_MAX, sizeof(sh_int));
    pcdata->alias = calloc(MAX_ALIAS, sizeof(char *));
    pcdata->alias_sub = calloc(MAX_ALIAS, sizeof(char *));
...
DEFINE_DISPOSE_FUN(pcdata_dispose)
{
    ...
    free(pcdata->alias);
    free(pcdata->alias_sub);
    free(pcdata->learned);
    free(pcdata->skill_known);
    free(pcdata->group_known);
...
{
    SHOP_T *shop = obj;
    shop->buy_type = calloc(MAX_TRADE, sizeof(sh_int));
    shop->buy_count = MAX_TRADE;
    shop->profit_buy = 100;
