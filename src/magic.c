...
int find_spell(CHAR_T *ch, const char *name)
{
    ...
    for (sn = 0; sn < SKILL_MAX; sn++)
    {
        ...
        if (LOWER(name[0]) == LOWER(skill_table[sn].name[0]) && !str_prefix(name, skill_table[sn].name))
        {
            if (found == -1)
                found = sn;
            if (ch->level >= skill_table[sn].classes[ch->class].level && ch->pcdata->learned[sn] > 0)
                return sn;
        }
    }
    return found;
...
void say_spell_name(CHAR_T *ch, const char *name, int class)
{
    ...
    printf_to_char(ch, "{5You utter the word%s, {Y'%s'{x{5.{x\n\r", plural, name);
    snprintf(buf1, sizeof(buf1), "{5$n utters the word%s, {Y'%s'{x{5.{x", plural, name);

    plural = (strchr(words, ' ')) ? "s" : "";
    snprintf(buf2, sizeof(buf2), "{5$n utters the word%s, {Y'%s'{x{5.{x", plural, words);
