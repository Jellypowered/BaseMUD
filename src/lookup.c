...
SIMPLE_INDEX_BUNDLE_DYNAMIC(class, CLASS_T);
SIMPLE_INDEX_BUNDLE_DYNAMIC(liq, LIQ_T);
SIMPLE_INDEX_BUNDLE_DYNAMIC(pc_race, PC_RACE_T);
SIMPLE_INDEX_BUNDLE_DYNAMIC(race, RACE_T);
SIMPLE_INDEX_BUNDLE_DYNAMIC(skill, SKILL_T);
SIMPLE_INDEX_BUNDLE_DYNAMIC(skill_group, SKILL_GROUP_T);
...
const char *map_lookup_get_string(int index, flag_t value)
{
    ...
    switch (lookup->index)
    {
    case MAP_LOOKUP_WEAPON_TYPE:
        return weapon_get_name(value);
    case MAP_LOOKUP_ATTACK_TYPE:
        return attack_get_name(value);
    case MAP_LOOKUP_SKILL:
        return skill_get_name(value);
    case MAP_LOOKUP_LIQUID:
        return liq_get_name(value);
...
flag_t map_lookup_get_type(int index, const char *str)
{
    ...
    {
    case MAP_LOOKUP_WEAPON_TYPE:
        return weapon_lookup(str);
    case MAP_LOOKUP_ATTACK_TYPE:
        return attack_lookup(str);
    case MAP_LOOKUP_SKILL:
        return skill_lookup(str);
    case MAP_LOOKUP_LIQUID:
        return liq_lookup(str);
