...
void bust_a_prompt(CHAR_T *ch)
{
    ...
    while (*str != '\0')
    {
        ...
        case 'h':
        {
            int percent = (ch->max_hit > 0) ? (100 * ch->hit / ch->max_hit) : 100;
            if (percent >= 80)
                strcpy(buf2, "{G"); // green
            else if (percent >= 60)
                strcpy(buf2, "{Y"); // yellow
            else if (percent >= 40)
                strcpy(buf2, "{y"); // dull yellow
            else if (percent >= 20)
                strcpy(buf2, "{R"); // red
            else
                strcpy(buf2, "{D"); // dark grey
            sprintf(buf2 + strlen(buf2), "%d{x", ch->hit);
            i = buf2;
            break;
        }
        case 'H':
        ...
        case 'm':
        {
            int percent = (ch->max_mana > 0) ? (100 * ch->mana / ch->max_mana) : 100;
            if (percent >= 80)
                strcpy(buf2, "{B"); // blue
            else if (percent >= 60)
                strcpy(buf2, "{C"); // cyan
            else if (percent >= 40)
                strcpy(buf2, "{c"); // dull cyan
            else if (percent >= 20)
                strcpy(buf2, "{M"); // magenta
            else
                strcpy(buf2, "{D"); // dark grey
            sprintf(buf2 + strlen(buf2), "%d{x", ch->mana);
            i = buf2;
            break;
        }
        case 'M':
        ...
        case 'v':
        {
            int percent = (ch->max_move > 0) ? (100 * ch->move / ch->max_move) : 100;
            if (percent >= 80)
                strcpy(buf2, "{Y"); // yellow
            else if (percent >= 60)
                strcpy(buf2, "{G"); // green
            else if (percent >= 40)
                strcpy(buf2, "{g"); // dull green
            else if (percent >= 20)
                strcpy(buf2, "{R"); // red
            else
                strcpy(buf2, "{D"); // dark grey
            sprintf(buf2 + strlen(buf2), "%d{x", ch->move);
            i = buf2;
            break;
        }
        case 'V':
        ...
        case 'X':
            sprintf(buf2, "%d", IS_NPC(ch) ? 0 : (ch->level + 1) * player_get_exp_per_level(ch) - ch->exp);
            i = buf2;
            break;
            ...
            flag_t flag, flag_t flag_skip, int min_level)
{
    ...
    {
        if (d->connected == CON_PLAYING && IS_IMMORTAL(d->character) && IS_SET(d->character->wiznet, WIZ_ON) && (!flag || IS_SET(d->character->wiznet, flag)) && (!flag_skip || !IS_SET(d->character->wiznet, flag_skip)) && char_get_trust(d->character) >= min_level && d->character != ch)
        {
