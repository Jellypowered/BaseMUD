...
int mana_gain(CHAR_T *ch, bool apply_learning)
{
    ...
    {
        gain = (char_get_curr_stat(ch, STAT_WIS) + char_get_curr_stat(ch, STAT_INT) + ch->level) / 2;
    ...
    for (wch = char_first; wch != NULL; wch = wch_next)
    {
        ...
        for (ch = wch->in_room->people_first; ch != NULL; ch = ch_next)
        {
            ...
            for (vch = wch->in_room->people_first; vch != NULL; vch = vch_next)
            {
                ...
                if (!IS_NPC(vch) && vch->level < LEVEL_IMMORTAL && ch->level >= vch->level - 5 && (!EXT_IS_SET(ch->ext_mob, MOB_WIMPY) || !IS_AWAKE(vch)) && char_can_see_in_room(ch, vch))
                {
