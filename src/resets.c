...
void reset_run(RESET_T *reset)
{
    ...
    case 'O':
    {
        ...
        if (room_index->area->nplayer > 0 ||
            obj_index_count_in_list(obj_index, room_index->content_first) >= obj_count)
        {
            reset_last_created = FALSE;
            return;
        }
    ...
    case 'E':
    {
        ...
        if (reset_last_mob->mob_index->shop)
        {
            int olevel = (obj_index->new_format)
                             ? 0
                             : item_index_get_old_reset_shop_level(obj_index);
...
/* Scoped version of reset_commit_all() for a single area. */
void reset_commit_area(AREA_T *area)
{
    RESET_T *reset, *next;
    for (reset = reset_data_get_first(); reset != NULL; reset = next)
    {
