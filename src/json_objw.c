...
JSON_T *json_objw_shop(const char *name, const SHOP_T *shop)
{
    ...
    for (i = 0; i < shop->buy_count; i++)
    {
        const char *name = shop->buy_type[i] <= 0 ? NULL : item_get_name(shop->buy_type[i]);
        if (name == NULL)
            continue;
...
JSON_T *json_objw_affect(const char *name, const AFFECT_T *aff)
{
    ...
    {
        json_prop_string(new, "apply", type_get_name(affect_apply_types, aff->apply));
...
JSON_T *json_objw_table(const char *name, const TABLE_T *table)
{
    ...
    {
        const void *obj = (table->table_pp != NULL)
                              ? *table->table_pp
                              : table->table;
        JSON_T *json;
