...
void fwrite_char(CHAR_T *ch, FILE *fp)
{
    ...
    fprintf(fp, "Scro %d\n", ch->lines);
    fprintf(fp, "Room %d\n", (ch->in_room == room_get_index(ROOM_VNUM_LIMBO) && ch->was_in_room != NULL) ? ch->was_in_room->vnum : ch->in_room == NULL ? ROOM_VNUM_TEMPLE
                                                                                                                                                       : ch->in_room->vnum);
    ...
    if (ch->affected_by != 0)
        fprintf(fp, "AfBy %s\n", fwrite_flags_static(affect_flags, ch->affected_by));
...
void fwrite_pet(CHAR_T *pet, FILE *fp)
{
    ...
    if (!EXT_EQUALS(pet->ext_mob, mob_index->ext_mob_final))
        fprintf(fp, "Mob  %s\n", fwrite_ext_flags_static(mob_flags, pet->ext_mob));
    if (EXT_IS_NONZERO(pet->ext_plr))
        fprintf(fp, "Plr  %s\n", fwrite_ext_flags_static(plr_flags, pet->ext_plr));
    if (pet->affected_by != mob_index->affected_by_final)
        fprintf(fp, "AfBy %s\n", fwrite_flags_static(affect_flags, pet->affected_by));
    if (pet->comm != 0)
        fprintf(fp, "Comm %s\n", fwrite_flags_static(comm_flags, pet->comm));
