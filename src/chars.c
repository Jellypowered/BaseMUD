...
bool char_can_see_anywhere(const CHAR_T *ch, const CHAR_T *victim)
{
    ...
    if (char_get_trust(ch) < victim->incog_level &&
        ...
        return FALSE;
    if ((!IS_NPC(ch) && EXT_IS_SET(ch->ext_plr, PLR_HOLYLIGHT)) || (IS_NPC(ch) && IS_IMMORTAL(ch)))
        return TRUE;
    ...
    /* sneaking */
    if (IS_AFFECTED(victim, AFF_SNEAK) && !IS_AFFECTED(ch, AFF_DETECT_HIDDEN) && victim->fighting == NULL && victim->position > POS_STUNNED)
    {
        int chance;
...
void char_move(CHAR_T *ch, int door, bool follow)
{
    ...
    for (fch = in_room->people_first; fch != NULL; fch = fch_next)
    {
        ...
        if (fch->master == ch && fch->position == POS_STANDING && char_can_see_room(fch, to_room))
        {

            if (IS_SET(ch->in_room->room_flags, ROOM_LAW) && (IS_NPC(fch) && EXT_IS_SET(fch->ext_mob, MOB_AGGRESSIVE)))
            {
...
char *char_format_to_char(const CHAR_T *victim, const CHAR_T *ch)
{
    ...
    // Add quest target indicator for quest mobs
    if (IS_NPC(victim) && ch->questmob > 0 && victim->mob_index && victim->mob_index->vnum == ch->questmob)
    {
#ifdef BASEMUD_COLOR_STATUS_EFFECTS
...
void char_update(CHAR_T *ch)
{
    ...
    if (ch->position >= POS_STUNNED)
    {
        ...
        if (IS_NPC(ch) && ch->area != NULL && ch->area != ch->in_room->area && ch->desc == NULL && ch->fighting == NULL && !IS_AFFECTED(ch, AFF_CHARM) && number_percent() < 5)
        {
