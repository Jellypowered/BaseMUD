...
#define CAF current_area_file
            if (!str_cmp(word, "AREA"))
                ...
                load_area_olc(CAF); /* OLC */
            else if (!str_cmp(word, "HELPS"))
                load_helps(CAF, current_area_filename);
                ...
                case 0:
                    ...
                    if (pexit->key >= KEY_VALID)
                        bugf("Warning: Room %d with non-door exit %d has "
                             "key %d",
                             room_index->vnum, door, pexit->key);
#endif
...
void load_socials(FILE *fp)
{
    while (1)
    {
        ...
        do
        {
            if (!load_socials_string(fp, &(new->char_no_arg)))
                break;
            if (!load_socials_string(fp, &(new->others_no_arg)))
                break;
            if (!load_socials_string(fp, &(new->char_found)))
                break;
            if (!load_socials_string(fp, &(new->others_found)))
                break;
            if (!load_socials_string(fp, &(new->vict_found)))
                break;
            if (!load_socials_string(fp, &(new->char_not_found)))
                break;
            if (!load_socials_string(fp, &(new->char_auto)))
                break;
            if (!load_socials_string(fp, &(new->others_auto)))
                break;
        } while (0);
...
void load_objects(FILE *fp)
{
    ...
    while (1)
    {
        ...
        case ('P'):
            ...
            break;
        case ('G'):
            obj_index->condition = 90;
            break;
        case ('A'):
            obj_index->condition = 75;
            break;
        case ('W'):
            obj_index->condition = 50;
            break;
        case ('D'):
            obj_index->condition = 25;
            break;
        case ('B'):
            obj_index->condition = 10;
            break;
        case ('R'):
            obj_index->condition = 0;
            break;
        default:
            obj_index->condition = 100;
            break;
        ...
        while (1)
        {
            ...
            else if (letter == 'F')
            {
                ...
                {
                case 'A':
                    paf->bit_type = AFF_TO_AFFECTS;
                    break;
                case 'I':
                    paf->bit_type = AFF_TO_IMMUNE;
                    break;
                case 'R':
                    paf->bit_type = AFF_TO_RESIST;
                    break;
                case 'V':
                    paf->bit_type = AFF_TO_VULN;
                    break;
                default:
                    EXIT_IF_BUG(TRUE,
